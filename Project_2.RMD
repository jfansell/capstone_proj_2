...
```{r train control for red wine}
train_red_ctrl <- trainControl(method = "cv",
                               number = 10)
```


#### White wine data splitting  

Splitting the white wine into a test and train partition will be done on the 80% train, and 20% test split as the data set is considerably larger with `r nrow(white_wine)` samples in the data set.

```{r Split white wine, warning=FALSE}
set.seed(1236, sample.kind = "Rounding")
...
The white wine test set is `r nrow(white_wine_test)` and the training data is a quite hefty `r nrow(white_wine_train)`. This could lead to some model training taking a little longer. The validation of the white wine training set can be reduced in comparison to the red wine.
...
```{r Train control for white wine}
...
#### LDA model for red wine quality  

Fitting and training the first model for wine quality is done very simply with the following code.
...
```{r Collect results LDA, clear environment, warning=FALSE, include=FALSE}
...
#### LDA for white wine quality  

Once again, as for the red wine, the model is quite easily fitted using the interface provided by caret. Obtaining results is also as straightforward.

```{r LDA white wine and predictions, warning=FALSE}
...
```{r Update results, clear envir., warning=FALSE, include=FALSE}
...
A random forest is an ensemble method for machine learning. It is an extension of the classic decision tree - multiple decision trees are built using a random selection of the feature variables. This allows the classification trees to be smaller (and thus less prone to over training). The number of trees is allowed to grow and is frequently around 500 different trees which "vote" on the final classification. There are a number of random forest implentations available in R. We will use the randomForest package implementation. This has one tuneable hyperparameter - "mtry" the number of variables randomly sampled at each tree split. The default value is the square root of the number of feature variables.

#### Random forest for red wine prediction

Once again caret performs handles the fitting and training of the predictive models with ease. For the initial run the hyperparameter will be left to its default values on which caret trains the models. As this is a stochastic model a seed will need to be specified to obtain reproducible results.

```{r red wine random forest, warning=FALSE}
set.seed(1236, sample.kind = "Rounding")
...
```{r Mtry results for default RF}
red_wine_rf$results[,1:2] %>%
  knitr::kable(caption = "Default tuning parameters for red wine random forest") %>%
  kable_styling(latex_options = "HOLD_position")
```
```{r Collect results def_RF, message=FALSE, warning=FALSE, include=FALSE}
quality_results <- bind_rows(quality_results, tibble(Model = "Default RF", Wine_type = "Red", Accuracy = results_red_rf$overall[1]))
```


The graph of these these values shows the best value of "mtry" as being 10. However, only three points have been sampled and it is possible that being trying further values between 2-10 that a different local maximum accuracy could be found.

```{r Graph default values of mtry, echo=FALSE, message=FALSE, warning=FALSE}
 red_wine_rf$results[,1:2] %>%
...
```{r Clean up enviroment, message=FALSE, warning=FALSE, include=FALSE}
...
```{r Expand grid search for RF-red wine, message=FALSE, warning=FALSE}
set.seed(1236, sample.kind = "Rounding")
...
```{r mtry values tabulated, echo=FALSE, message=FALSE, warning=FALSE}
red_wine_tunedRF$results[,1:2] %>%
  knitr::kable(caption = "Exapnded mtry results") %>%
  kable_styling(latex_options = "HOLD_position")
```
A graph depicting these results will show how the accuracy moved around under different values of mtry.

```{r Graph mtry expanded, echo=FALSE}
red_wine_tunedRF$results[,1:2] %>%
  ggplot(aes(mtry, Accuracy)) + 
...
varImpPlot(red_wine_tunedRF$finalModel)
```
...
```{r Collect results and clear envir., message=FALSE, warning=FALSE, include=FALSE}
...
#### Random forest for white wine quality prediction

Before proceeding with the modelling process for the white wine, given the poor performance of the LDA, it would be appropriate to amend the training control. The cross validation will be increased to ten fold to give a model better chance to find better solutions. This will add to computational burden.

```{r Update train control for white wine}
train_white_ctrl <- trainControl(method = "cv",
                                 number = 10)
```
...
```{r Random forest for white wine, warning=FALSE}
set.seed(1236, sample.kind = "Rounding")
...
```{r Collect results white wine RF, include=FALSE}
quality_results <- bind_rows(quality_results,
...
```{r Mtry vs accuracy, message=FALSE, warning=FALSE}
white_wine_rf$results[,1:2] %>%
  knitr::kable(caption = "Mtry vs cross validation accuracy") %>%
  kable_styling(latex_options = "HOLD_position")
```

```{r Graph accuracy vs mtry, echo=FALSE}
white_wine_rf$results[,1:2] %>%
  ggplot(aes(mtry, Accuracy)) + 
...
```{r Clear local storage , include=FALSE}
...
```{r Tuned rf for white wine, warning=FALSE}
set.seed(1236, sample.kind = "Rounding")
...
There has been a marginal improvement in performance. Encourangingly for such a tightly bound data range most values in the confusion matrix are clustered around the main diagonal.

```{r Update results tunedRF white, include=FALSE}
quality_results <- bind_rows(quality_results,
                             tibble(Model = "Tuned RF",
                                    Wine_type = "White",
...
[^11]: https://www.compoundchem.com/2014/05/28/redwinechemicals
